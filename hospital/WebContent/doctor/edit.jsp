...
<span style="font-size: 20px; color:red;">${msg }</span>
	<!-- 
		did的传递是为了标识医生，只改变当前医生的信息
		func是当前修改医生在Servlet中执行的方法
	 -->
<form action="${path }doctor" method="post" class="definewidth m20">
<input type="hidden" name="did" value="${doctor.did }"/>
<input type="hidden" name="func" value="updateDoctor"/>
<table class="table table-bordered table-hover definewidth m10">
	
    <tr>
    
        <td width="10%" class="tableleft">姓名</td>
        <td><input type="text" id="dname" name="dname" value="${doctor.dname }"/> <span id="dnameSpan"></span></td>
       
    </tr>
    
    <tr>
        <td width="10%" class="tableleft">身份证号</td>
        <td><input type="text" id="cardno" name="cardno" value="${doctor.cardno }"/><span id="cardnoSpan"></span></td>
    </tr>
    <tr>
        <td width="10%" class="tableleft">手机</td>
        <td><input type="text" id="phone" name="phone" value="${doctor.phone }"/><span id="phoneSpan"></span></td>
    </tr>
    <tr>
        <td width="10%" class="tableleft">性别</td>
        <td>
        	<input type="radio" <c:if test="${doctor.gender == 1 }">checked</c:if> name="gender" value="1" />男&nbsp;&nbsp;&nbsp;
        	<input type="radio" <c:if test="${doctor.gender == 0 }">checked</c:if> name="gender"  value="0" />女
        </td>
    </tr>
    <tr>
        <td width="10%" class="tableleft">出生年月</td>
        <td><input type="text" id="birthday" name="birthday" class="Wdate" 
        onclick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})"
        value="<fmt:formatDate value="${doctor.birthday }" pattern="yyyy-MM-dd HH:mm:ss"/>"/><span id="birthdaySpan"></span></td>
    </tr>
    <tr>
        <td width="10%" class="tableleft">年龄</td>
        <td><input type="text" id="age" name="age" value="${doctor.age }"/><span id="ageSpan"></span></td>
    </tr>
    <tr>
        <td width="10%" class="tableleft">电子邮箱</td>
        <td><input type="text" id="email" name="email" value="${doctor.email }"/><span id="emailSpan"></span></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
        	<select name="degree">
        		<option value="1" <c:if test="${doctor.degree == 1 }">selected</c:if>>专科</option>
        		<option value="2" <c:if test="${doctor.degree == 2 }">selected</c:if>>本科</option>
        		<option value="3" <c:if test="${doctor.degree == 3 }">selected</c:if>>研究生</option>
        		<option value="4" <c:if test="${doctor.degree == 4 }">selected</c:if>>博士</option>
        	</select>
        </td>
    </tr>
	<tr>
        <td width="10%" class="tableleft">备注</td>
        <td><textarea name="mark">${doctor.mark }</textarea></td>
	</tr>
    <tr>
        <td colspan="2">
			<center>
